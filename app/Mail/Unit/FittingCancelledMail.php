...
/**
 * Passing/service cancellation for assigned advisors and mechanics. Template placeholders include [user_first_name],
 ...
 */
class FittingCancelledMail extends Mailable
{
    ...
    {
        $this->order->loadMissing(['customer', 'billingCustomer', 'advisor', 'activeFittingAppointment']);

        $cancelReason = $this->order->getCancelComment();
        $appointmentComment = $this->resolveLatestFittingAppointment()?->comment;

        $resolvedReason = $this->reason ?? $cancelReason ?? $appointmentComment ?? '';

        $recipient = $this->resolveRecipientUser();

        return array_merge(
            $this->buildFittingAppointmentTemplateVars([
