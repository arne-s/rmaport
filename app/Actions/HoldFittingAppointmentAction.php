...
class HoldFittingAppointmentAction
{
    ...
    {
        if ($main->is_completed || ! $main->canModifyFittingAppointment()) {
            return;
        }
        ...
        $main->changeOrderStatus(OrderStatus::FittingOnHold);

        // Cancellation notifications ignore notify_advisor / notify_workshop / notify_customer on the appointment.
        app(SendFittingCancelledMailAction::class)->execute($main, $reason, $appointment);

        $main->orderEvents()->create([
            'type'    => 'Passing on hold: opnieuw inplannen',
            'data'    => [
