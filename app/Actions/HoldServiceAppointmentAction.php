...
class HoldServiceAppointmentAction
{
    ...
    public function execute(Main $main, string $reason): void
    {
        ...
        $main->changeOrderStatus(OrderStatus::AssemblyOnHold);

        // Cancellation notifications ignore notify_advisor / notify_workshop / notify_customer on the appointment.
        app(SendServiceCancelledMailAction::class)->execute($main, $reason, $appointment);

        $main->orderEvents()->create([
            'type'    => 'Montage on hold: opnieuw inplannen',
            'data'    => [
