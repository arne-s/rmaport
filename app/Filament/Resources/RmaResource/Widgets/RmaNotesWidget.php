...
namespace App\Filament\Resources\RmaResource\Widgets;
...
class RmaNotesWidget extends TableWidget
{
    ...
    {
        if (! $this->record instanceof Rma) {
            return [];
        }

        $defaults = [
            'attachments_bucket' => (string) Str::uuid(),
            'type' => NoteType::Rma->value,
            'rma_id' => $this->record->getKey(),
        ];

        if ($this->record->customer_id !== null) {
            $defaults['customer_id'] = $this->record->customer_id;
            $defaults['customer_or_company'] = 'customer-'.$this->record->customer_id;
        }
    ...
    protected function getTableQuery(): Builder
    {
        ...
        return Note::query()
            ->whereHas('rmas', fn (Builder $query): Builder => $query->whereKey($this->record->getKey()))
            ->with(['user', 'customer']);
    }
    ...
    public function table(Table $table): Table
    {
        return $table
            ...
            ->emptyStateHeading('Geen notities voor deze RMA')
            ->paginated([10, 25, 50])
