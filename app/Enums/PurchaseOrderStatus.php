...
enum PurchaseOrderStatus: string
{
    ...
    /** Saved stock-order concept (replaces {@see self::Initial} for stock orders). */
    case Draft = 'draft';
    ...
    public function getLabel(): ?string
    {
        return match ($this) {
            ...
            self::Cancelled => 'Geannuleerd', // todo: can this be removed?
            default => null,
        };
    }
    ...
    /**
     * Line status to apply when this PO status is set on the header (null = do not bulk-sync lines).
     */
    public function toOrderProductStatus(): ?OrderProductStatus
    {
        return match ($this) {
            self::Purchased => OrderProductStatus::Purchased,
    ...
    public static function getCategory(self $status): string
    {
        return match ($status) {
            self::Draft => 'Concept',
            self::Purchased, self::PartiallyConfirmed, self::Confirmed => 'Inkopen',
            self::PartiallyDelivered, self::Delivered => 'Op locatie',
