...
{
    const DOCUMENT_TYPE_PURCHASE_ENTRY = 20; // Purchase Entry Document Type

    const DOCUMENT_CATEGORY_ID_SUPPLIERS = '78eae616-0212-4abd-bc9e-c5fd4b80b372'; // Document Category: Suppliers / Crediteuren
    const UNITS = [
        // Pieces
        'pc' => [
    ...
    public function getPaidPurchaseInvoices(array $ids): ?array
    {
        if (empty($ids)) {
            return [];
        }

        $selectFields = 'TransactionEntryID,EntryDate';
        // Status 50 = matched (afgeletterd)
        $filter = "Status eq 50";
        if (!empty($ids)) {
            $idFilters = array_map(fn($id) => "TransactionEntryID eq guid'{$id}'", $ids);
            $filter .= ' and (' . implode(' or ', $idFilters) . ')';
        }

        $url = $this->url("v1/{$this->division}/cashflow/Payments?\$select={$selectFields}&\$filter={$filter}");
        ...
        try {
            ...
            $response = $e->getResponse();
            $this->log("Error occured while fetching paid purchase invoices: {$response?->getStatusCode()} {$response?->getBody()?->getContents()}", 'error');
        } catch (Throwable $e) {
            $this->log('Exact Online API error: ' . (string)$e, 'error');
        }
        return null;
    ...
    /**
     * Retrieves a purchase entry from Exact Online by its EntryID.
     *
     * @param string $entryId The EntryID of the purchase entry to retrieve.
     * @return array|null The purchase entry data if found, otherwise null.
     */
    public function getPurchaseEntry(string $entryId): ?array
    {
        $url = $this->url("v1/{$this->division}/purchaseentry/PurchaseEntries(guid'{$entryId}')");
        ...
        try {
            ...
            $response = $e->getResponse();
            $this->log("Error retrieving purchase entry: {$response?->getStatusCode()} {$response?->getBody()?->getContents()}", 'error');
        } catch (Throwable $e) {
            $this->log('Exact Online API error: ' . (string)$e, 'error');
        }
    ...
    /**
     * Find an existing purchase entry in Exact Online for the same supplier and invoice reference.
     *
     * @return array<string, mixed>|null
     */
    public function findExistingPurchaseEntry(string $supplierExactId, string $yourRef, ?string $description = null): ?array
    {
        if (blank($supplierExactId)) {
            return null;
        }

        if (filled($yourRef)) {
            $entry = $this->queryPurchaseEntriesByFilter(sprintf(
                "Supplier eq guid'%s' and YourRef eq '%s'",
                $supplierExactId,
                $this->escapeODataString($yourRef),
            ));
        ...
        if (filled($description)) {
            return $this->queryPurchaseEntriesByFilter(sprintf(
                "Supplier eq guid'%s' and Description eq '%s'",
                $supplierExactId,
                $this->escapeODataString($description),
            ));
        }
     ...
     * @return array<string, mixed>|null
     */
    private function queryPurchaseEntriesByFilter(string $filter): ?array
    {
        $url = $this->url("v1/{$this->division}/purchaseentry/PurchaseEntries");
        ...
        try {
            $response = $this->client->get($url, [
                ...
                'query' => [
                    '$select' => 'EntryID,YourRef,Description,AmountDC,Supplier',
                    '$filter' => $filter,
                    '$top' => 2,
                ],
            ]);
            ...
            if (count($results) > 1) {
                $this->log("Multiple purchase entries found for filter: {$filter}", 'warning');
            }

            return $results[0];
        } catch (BadResponseException $e) {
            $response = $e->getResponse();
            $this->log("Error searching purchase entries: {$response?->getStatusCode()} {$response?->getBody()?->getContents()}", 'error');
        } catch (Throwable $e) {
            $this->log('Exact Online API error while searching purchase entries: ' . (string) $e, 'error');
        }
    ...
    /**
     * Create a purchase entry in Exact Online, including its lines.
     *
     * @param array $data Purchase entry data. Should include 'PurchaseEntryLines' key for lines.
     * @return string|null Entry ID if created, null otherwise.
     */
    public function createPurchaseEntry(array $data): ?string
    {
        $url = $this->url("v1/{$this->division}/purchaseentry/PurchaseEntries");
        ...
        try {
            ...
            $result = json_decode($response->getBody()->getContents(), true);
            if (isset($result['d']['EntryID'])) {
                return $result['d']['EntryID'];
            }
        } catch (BadResponseException $e) {
            $response = $e->getResponse();
            $this->log("Error occured while creating purchase entry: {$response?->getStatusCode()} {$response?->getBody()?->getContents()}", 'error');
        } catch (Throwable $e) {
            $this->log('Exact Online API error: ' . (string)$e, 'error');
        }
        return null;
    ...
    /**
     * Creates a purchase invoice in Exact Online.
     * This method handles the creation of a purchase invoice, including the associated document and attachment.
     * @param PurchaseOrderInvoice $invoice The purchase order invoice to create in Exact Online.
     * @return string|bool|null Returns the purchase entry ID if successful, false if not connected, or null if an error occurred.
     */
    public function createPurchaseInvoice(PurchaseOrderInvoice $invoice): string|bool|null
    {
        $this->log("Creating purchase invoice in Exact Online for invoice ID: {$invoice->id} ({$invoice->invoice_number})");
        ...
        $purchaseOrder = $invoice->orderable;
        if (! $purchaseOrder instanceof \App\Models\PurchaseOrder || empty($purchaseOrder->reference_number)) {
            $this->log("Purchase order reference number is empty for invoice ID: {$invoice->id} ({$invoice->invoice_number})", 'error');
        ...
        if (empty($supplier->exact_id)
            || empty($supplier->exact_gl_account_id)
            || empty($supplier->exact_vat_code_id)
            || empty($supplier->exact_payment_condition_id)) {
            $this->log("Supplier {$supplier->id} is not properly configured for Exact Online sync", 'error');

            return false;
        }

        $pdfPath = $invoice->getPdfAbsolutePath();
        if ($pdfPath === null || ! is_file($pdfPath)) {
            $this->log("PDF not found for purchase invoice ID: {$invoice->id}", 'error');

            return false;
        }

        $existingEntry = $this->findExistingPurchaseEntry(
            $supplier->exact_id,
            $invoice->invoice_number,
            $purchaseOrder->reference_number,
        );

        if ($existingEntry !== null && filled($existingEntry['EntryID'] ?? null)) {
            $entryId = (string) $existingEntry['EntryID'];

            $this->resolvePurchaseInvoiceExactIdConflict($invoice, $entryId);

            if (! $invoice->exists) {
                $this->log(
                    "Purchase invoice {$invoice->invoice_number} merged into existing Exact import record (Entry ID: {$entryId})",
                    'info',
                );
            ...
            $this->log(
                "Purchase invoice {$invoice->id} ({$invoice->invoice_number}) already exists in Exact Online, linked Entry ID: {$entryId}",
                'info',
            );
        ...
        $invoice->forceFill([
            'exact_error_at' => now(),
            'exact_error_message' => 'Synchronisatie naar Exact Online mislukt',
        ])->save();

        $paymentDays = (int) ($supplier->exactPaymentCondition?->payment_days ?? 0);
        $entryDate = $invoice->entry_date;
        $dueDate = $invoice->due_date?->format('Y-m-d')
            ?? $entryDate->copy()->addDays($paymentDays)->format('Y-m-d');
        $purchaseEntryData = [
            'Journal' => (string) config('exact.purchase_journal_code', '30'),
            'Supplier' => $supplier->exact_id,
            'Description' => $purchaseOrder->reference_number,
            'YourRef' => $invoice->invoice_number,
            'EntryDate' => $entryDate->format('Y-m-d'),
            'DueDate' => $dueDate,
            'PaymentCondition' => $supplier->exactPaymentCondition?->code,
            'PurchaseEntryLines' => [[
                'GLAccount' => $supplier->exactGlAccount?->guid,
                'AmountFC' => $invoice->amount,
                'Description' => $purchaseOrder->reference_number,
                'VATCode' => $supplier->exactVatCode?->code,
            ]],
        ];

        try {
            $documentId = $this->createDocument([
                'Type' => self::DOCUMENT_TYPE_PURCHASE_ENTRY,
                'Subject' => $purchaseOrder->reference_number,
                'Account' => $supplier->exact_id,
                'Category' => self::DOCUMENT_CATEGORY_ID_SUPPLIERS,
                'DocumentDate' => $entryDate->format('Y-m-d'),
                'AmountFC' => $invoice->total_amount_inc_vat,
            ]);
            if (! $documentId) {
                $this->log('Failed to create document for purchase invoice in Exact Online.', 'error');

                return null;
            }

            $purchaseEntryData['Document'] = $documentId;

            $attachmentId = $this->uploadDocumentAttachment($documentId, $pdfPath);
            if (! $attachmentId) {
                $this->log('Failed to upload document attachment for purchase invoice in Exact Online.', 'error');
                $this->deleteResource('documents/Documents', $documentId);

                return null;
            }

            $purchaseEntryId = $this->createPurchaseEntry($purchaseEntryData);
            if (! $purchaseEntryId) {
                $this->log('Failed to create purchase entry in Exact Online.', 'error');
                $this->deleteResource('documents/DocumentAttachments', $attachmentId);
                $this->deleteResource('documents/Documents', $documentId);
            ...
            $this->log("Purchase invoice {$invoice->id} ({$invoice->invoice_number}) created successfully in Exact Online with Entry ID: {$purchaseEntryId}", 'info');

            return $purchaseEntryId;
        } catch (Throwable $e) {
            $this->log('Error with purchase invoice in Exact Online: ' . (string) $e, 'error');
    ...
    /**
     * When Exact already has an entry that was imported separately, avoid duplicate exact_id rows.
     */
    private function resolvePurchaseInvoiceExactIdConflict(PurchaseOrderInvoice $invoice, string $entryId): void
    {
        $conflicting = PurchaseOrderInvoice::query()
            ->where('exact_id', $entryId)
        ...
        $invoiceLinked = $invoice->isLinkedToPurchaseOrder();
        $conflictingLinked = $conflicting->isLinkedToPurchaseOrder();

        if ($invoiceLinked && $conflictingLinked
            && (int) $invoice->orderable_id === (int) $conflicting->orderable_id) {
            $this->mergePurchaseOrderInvoiceAttributes($conflicting, $invoice);
            $this->deleteDuplicatePurchaseOrderInvoice($invoice);

            return;
        }

        if ($invoiceLinked && ! $conflictingLinked) {
            $this->deleteDuplicatePurchaseOrderInvoice($conflicting);
        ...
        throw new RuntimeException(
            "Exact entry {$entryId} is already linked to purchase order invoice ID {$conflicting->id}",
        );
    }

    private function mergePurchaseOrderInvoiceAttributes(
        PurchaseOrderInvoice $keeper,
        PurchaseOrderInvoice $duplicate,
    ): void {
        $updates = [];

        if (blank($keeper->description) && filled($duplicate->description)) {
            $updates['description'] = $duplicate->description;
        }

        foreach (['amount', 'vat_amount', 'total_amount_inc_vat', 'due_date', 'supplier_name', 'document_id'] as $field) {
            if ($keeper->{$field} === null && $duplicate->{$field} !== null) {
    ...
    private function deleteDuplicatePurchaseOrderInvoice(PurchaseOrderInvoice $invoice): void
    {
        $invoice->clearMediaCollection('documents');
        $invoice->delete();
    }

    /**
     * Deletes a purchase invoice in Exact Online, including its document and attachments.
     * @param string $entryId The EntryID of the purchase invoice to delete.
     * @return bool Returns true if the deletion was successful, false otherwise.
     */
    public function deleteFullPurchaseInvoice(string $entryId): bool
    {
        $this->log("Fully deleting purchase invoice in Exact Online for entry ID: {$entryId}");
        ...
        if (empty($entryId) || empty($purchaseEntry = $this->getPurchaseEntry($entryId))) {
            $this->log("No purchase entry found for entry ID: {$entryId}", 'error');
            return false;
        }

        $this->deleteResource('purchaseentry/PurchaseEntries', $entryId);

        if (!empty($documentId = $purchaseEntry['Document'] ?? null)) {
            if (!empty($attachments = $this->getDocumentAttachments($documentId))) {
                foreach ($attachments as $attachment) {
                    if (!empty($attachment['ID'])) {
                        $this->deleteResource('documents/DocumentAttachments', $attachment['ID']);
                    }
                }
            }
     ...
     */
    /**
     * Fetch purchase entries from Exact Online, optionally filtered by entry date.
     *
     * @param int $days Number of days back to filter on EntryDate. 0 = all entries.
     * @return array<int, array<string, mixed>>
     */
    public function getPurchaseEntries(int $days = 1): array
    {
        $url = $this->url("v1/{$this->division}/purchaseentry/PurchaseEntries");
        ...
        if ($days > 0) {
            ...
            $query['$filter'] = "EntryDate ge datetime'{$since}'";
        }
        ...
        try {
            ...
            $response = $e->getResponse();
            $this->log("Error retrieving purchase entries: {$response?->getStatusCode()} {$response?->getBody()?->getContents()}", 'error');
        } catch (Throwable $e) {
            $this->log('Exact Online API error (purchase entries): ' . (string) $e, 'error');
        }
