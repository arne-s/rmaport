@php
    ...
    $extraClass = is_string($extraAttributes['class'] ?? null) ? $extraAttributes['class'] : '';
    $wrapperClass = trim(($attributes->get('class') ?? '') . ' ' . $extraClass);
    $useRowDragHandle = str_contains($wrapperClass, 'orderProductsRepeater')
        && $isReorderableWithDragAndDrop
        && ! $isReorderableWithButtons;
    $showReorderColumn = (count($items) > 1)
        && ($isReorderableWithButtons || ($isReorderableWithDragAndDrop && ! $useRowDragHandle));
@endphp
    ...
    <div
        {{ $attributes
                ...
                ->class([
                    ...
                    'order-products-row-drag' => $useRowDragHandle,
                ]) }}
    >
        @if (count($items))
            <table>
                <thead>
                    <tr>
                        ...
                        @if (count($extraItemActions) || $isCloneable || $isDeletable || $isAddable)
                            <th
                                class="fi-fo-table-repeater-empty-header-cell"
                            ></th>
                        @endif
                ...
                >
                    @foreach ($items as $itemKey => $item)
                        @php
                            ...
                            $addBetweenAction = $addBetweenAction(['afterItem' => $itemKey]);
                            $addBetweenActionIsVisible = $isAddable && $addBetweenAction->isVisible();
                            ...
                            $reorderActionIsVisible = $isReorderableWithDragAndDrop && $reorderAction->isVisible() && ! $useRowDragHandle;
                        ...
                        >
                                ...
                                @if (count($tableColumns) > $counter)
                                    ...
                                    @else
                                        ...
                                        @if ($schemaComponent->isVisible())
                                            ...
                                            <td
                                                @class([
                                                    'fi-order-product-qty-column' => $useRowDragHandle && $counter === 1,
                                            ...
                                            >
                                                @if ($useRowDragHandle && $counter === 1)
                                                    <div class="fi-order-product-qty-drag-cell">
                                                        <span
                                                            class="fi-order-product-drag-handle"
                                                            x-sortable-handle
                                                            role="button"
                                                            tabindex="-1"
                                                            aria-label="{{ __('filament-forms::components.repeater.actions.reorder.label') }}"
                                                        >
                                                            <x-filament::icon
                                                                icon="heroicon-m-bars-3"
                                                                class="fi-order-product-drag-handle-icon"
                                                            />
                                                        </span>
                            ...
                            @if (count($extraItemActions) || $isCloneable || $isDeletable || $isAddable)
                                <td>
                                    @if ($visibleExtraItemActions || $cloneActionIsVisible || $deleteActionIsVisible || $addBetweenActionIsVisible)
                                        <div class="fi-fo-table-repeater-item-actions">
                                            ...
                                            @if ($addBetweenActionIsVisible)
                                                <div
                                                    class="fi-fo-table-repeater-add-between"
                                                    x-on:click.stop
                                                >
                                                    {{ $addBetweenAction }}
                                                </div>
