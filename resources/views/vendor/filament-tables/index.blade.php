@php
/** This file has been changed significantly please copy the filament file and compare changes when updating **/
    ...
    >
        ...
        >
            ...
            <div
                ...
                x-show="@js($hasHeaderToolbar) || @js($hasNonBulkToolbarAction) || @js($hasFiltersAboveContent) || @js(count($headerActions) > 0) || (getSelectedRecordsCount() && @js(count($toolbarActions)))"
                class="fi-ta-header-toolbar"
            >
                @if ($heading && ! $isReorderable)
                    <{{ $headingTag }}
                        class="fi-ta-header-heading customFilamentTableHeading dashboard-widget-toolbar-title"
                    >
                        {{ $heading }}
                    </{{ $headingTag }}>
                @endif
                @if ($isReorderable && $heading != null && $heading != '')
                    <div>
                        @if ($isReorderable)
                        ...
                        @if ($heading)
                            <{{ $headingTag }}
                                class="fi-ta-header-heading customFilamentTableHeading"
                            >
                                {{ $heading }}
                            </{{ $headingTag }}>
                        @endif
                ...
                <div class="fi-ta-actions fi-align-start fi-wrapped customFilterOptions">
                ...
                @if ($isGlobalSearchVisible || $hasFiltersDialog || $hasColumnManagerDropdown)
                    <div class="customFilamentSearch">
                        ...
                        @if ($isGlobalSearchVisible)
                            ...
                            <x-filament-tables::search-field
                                ...
                                {{-- Changes: Prevents a form from being saving when searching a table widget on an edit record page --}}
                ...
                @if ((! $isReordering) && $headerActions)
                    <div class="fi-ta-actions fi-align-start fi-wrapped customFilamentHeaderActions">
                        @foreach ($headerActions as $action)
                            {{ $action }}
                        @endforeach
                    </div>
